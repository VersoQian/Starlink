...
The user enters through a single prompt-first surface (Figure 1-2). From there three routes are available: submit the idea directly, open the knowledge-base path, or invoke the Coach wizard. Every route ultimately reaches the same canvas. The routes differ only in what preliminary context the canvas carries on arrival.

#### 4.3.2 Pre-Canvas: Knowledge Base and Coach

Two stages of pre-canvas reasoning are available before the supervisor's first dispatch, and they are independent: the user can use neither, either, or both. Both write their output into the cross-conversation memory layer at workspace scope, which the supervisor's initial-state assembly (Algorithm 1, line 1) reads on the first dispatch — the same `Update` primitive (Definition 2) handles both writes.
...
**The Ideation Coach** is responsible for the earlier stage of business analysis: helping the user articulate a viable idea before any canvas dimension is generated. The Coach is implemented as a standalone reasoning service that operates outside the LangGraph subgraph topology and outside the blackboard, on the design ground that ideation is conversational rather than structured and therefore does not benefit from a declarative-coverage mechanism. The Coach has two interaction modes that share infrastructure but differ in initiative.

The **reflective mode** is event-driven and user-initiated. It receives a free-form canvas event (a node added, a link drawn, a user message, an attempted cell edit) and returns a Socratic reflection that asks exactly one targeted follow-up question. The follow-up question is conditioned on inferred user-skill traits read from cross-conversation memory: a novice user receives a foundational question, while a user whose recent history indicates familiarity with the topic receives a question pitched one layer deeper.

The **wizard mode** is sequence-driven and system-initiated. It imposes a fixed seven-step sequence that walks the user from a core idea through customer pain, value angle, hypothesis, validation, revenue, and risk. A companion prefill component performs hybrid retrieval against any bound knowledge bases and pre-populates wizard answers where existing evidence already covers the step. The user can accept, edit, or reject each pre-filled answer. The wizard's final output is a structured ideation record — customer description, value proposition, hypothesis, validation plan, revenue model, identified risks — not a canvas.
...
The two modes hand off to the canvas core through a single integration point. At the end of an ideation session, the wizard's seven-step structured output — or, when no wizard was used, the accumulated reflective-mode exchanges — is materialised into the cross-conversation memory layer as a `summary` item at workspace scope, alongside any `user-skill` items capturing the traits the Coach has inferred. The Coach acts as a *preconditioning layer* on the main pipeline: it does not change the declarative-coverage mechanism, only the starting distribution of the analytical state.
