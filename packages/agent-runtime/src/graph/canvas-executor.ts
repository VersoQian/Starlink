import type { CanvasNode, CanvasEdge } from '@branching-chat/shared'
import type { AgentContext, CanvasExecutionResult, CanvasExecutionState } from './types.js'
import { buildInitialState, createNodeId, pushNode } from './initial-state.js'
import {
  MockKnowledgeBaseClient,
  type KnowledgeBaseClient,
  hydrateGraphWithKnowledge
...
async function addSubQuestions(state: CanvasExecutionState): Promise<CanvasExecutionState> {
...
async function addDimensions(state: CanvasExecutionState): Promise<CanvasExecutionState> {
...
async function addActionPlan(state: CanvasExecutionState): Promise<CanvasExecutionState> {
...
async function enrichWithKnowledge(
  state: CanvasExecutionState,
  knowledgeClient: KnowledgeBaseClient
): Promise<CanvasExecutionState> {
  ...
  if (knowledge.entries.length > 0) {
    const referenceNodes: CanvasNode[] = knowledge.entries.slice(0, 2).map((entry, index) => ({
    ...
    const edges: CanvasEdge[] = referenceNodes.map((node) => ({
