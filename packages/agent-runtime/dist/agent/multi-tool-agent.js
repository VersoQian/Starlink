...
import { MockKnowledgeBaseClient } from '../tools/knowledge-base.js';
import { createVisualizationAnalysisTool } from '../tools/visual-analysis.js';
function formatKnowledgeResult(result, limit) {
...
function formatGraphOverview(graph) {
...
export async function createStarlinkAgentExecutor(context) {
    ...
    if (process.env.DEEPSEEK_API_KEY) {
        ...
        console.warn('[starlink-agent] No LLM credentials detected. Falling back to MockChatModel (offline mode).');
        llm = new MockChatModel();
    }
    const searchKnowledgeTool = new DynamicStructuredTool({
        name: 'search_knowledge_base',
        description: '查询 Starlink 知识库以获取与当前议题相关的事实或参考资料。适用于补充论据、寻找引用、验证信息。',
        schema: z.object({
    ...
    const canvasInspectorTool = new DynamicStructuredTool({
        ...
        description: '查看当前 workspace 画布上的节点与连线，理解已经存在的拆解结构。在给出建议前使用，避免重复内容。',
        schema: z.object({
            focus: z
                .string()
    ...
    const taskPlannerTool = new DynamicStructuredTool({
        ...
        description: '根据当前议题生成建议的画布更新，例如新增节点、补充行动或引用。仅输出建议结构，不直接写入画布。',
        schema: z.object({
            goal: z
                .string()
    ...
    return initializeAgentExecutorWithOptions([searchKnowledgeTool, canvasInspectorTool, taskPlannerTool, visualizationTool], llm, {
        agentType: 'structured-chat-zero-shot-react-description',
        verbose: false
    });
}
export async function runStarlinkAgentTask(context, task) {
