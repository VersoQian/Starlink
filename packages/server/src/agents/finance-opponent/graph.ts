/**
 * Finance-opponent agent registration (orphan — see orphan-subgraph-stub.ts).
 *
...
const relevanceScorer: RelevanceScorer<OpponentRelevantState> = (state) => {
  ...
  const financeDisputed = state.conflicts.some(
    (c) =>
      c.severity === 'high' &&
      (c.relatedAgents ?? []).some((a) => a.toLowerCase().includes('finance'))
  )
  return financeDisputed ? 1 : 0
}
