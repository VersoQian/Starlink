/**
 * Product Agent Tool — analyzes CC-BMC dimensions:
 * value propositions, key resources, key activities, and key partnerships.
 */
...
const SYSTEM_PROMPT =
  '你是 Product_Agent（产品策略专家），负责分析 CC-BMC 商业模型画布中的四个维度：价值主张、核心资源、关键业务、重要合作。' +
  '根据用户问题生成结构化分析，每个维度包含 domain, content, confidence 字段。' +
  '以 JSON 格式输出 { "bmcCards": [...] }，每个元素包含 domain（维度名称）、content（分析内容）、confidence（0-1 置信度）。'

export default class ProductAgentTool extends BaseTool {
