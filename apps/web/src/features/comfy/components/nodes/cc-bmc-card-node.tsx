...
/** Human-readable relative time for the per-cell attribution chip.
 *  Updates 1Hz inside the card while the write is "fresh" (<10 min). */
...
/** Window (ms) during which the cell shows the "just-written" flash ring. */
...
export const CCBMCCardNode = memo(function CCBMCCardNode({ id, data }: NodeProps) {
  ...
  const writeInfo = useComfyStore((state) => state.nodeWrittenAt.get(id))
  const nodeData = macraNode || (data as MacraNodeData)

  // Tick to refresh "刚刚 / N 秒前 / N 分钟前" chip text + cycle off the
  // flash ring when its 5s window expires. 1Hz is cheap (one setState
  // per visible BMC card per second, ≤9 cards) and avoids requiring
  // every parent re-render to repaint these.
  const [, setTick] = useState(0)
  const writeAt = writeInfo?.at ?? 0
  useEffect(() => {
    if (writeAt === 0) return
    const sinceWrite = Date.now() - writeAt
    // No need to tick once the write is > 1 hour old — chip text
    // resolves to "N 小时前" which doesn't need 1Hz refresh.
    if (sinceWrite > 3_600_000) return
    const id = setInterval(() => setTick((n) => (n + 1) % 1000), 1000)
    return () => clearInterval(id)
  }, [writeAt])

  const writeAgeMs = writeAt > 0 ? Date.now() - writeAt : null
  const isFreshlyWritten = writeAgeMs !== null && writeAgeMs < FLASH_MS
  ...
  return (
    <>
      ...
      <div
        className={`group relative w-[340px] overflow-hidden border-[1px] bg-white transition-all duration-300 hover:border-stratum-blue/40 ${highlightClass} ${
          isFreshlyWritten ? 'border-stratum-blue/70' : 'border-stratum-line'
        }`}
        style={{
          // 3px left edge in muted byline color — semantic at-a-glance
          // owner cue (M/P/F) without the 9-color rainbow. When the cell
          // was just written by an agent, briefly amplify with a glow
          // ring in the byline color so "刚被改的 cell" is unmistakable.
          boxShadow: isFreshlyWritten
            ? `inset 3px 0 0 0 ${byline.edgeHex}, 0 0 0 2px ${byline.edgeHex}40, 0 0 16px ${byline.edgeHex}30`
            : `inset 3px 0 0 0 ${byline.edgeHex}`
        }}
      >
        ...
        <div className="flex items-baseline justify-between border-t-[0.5px] border-stratum-line px-4 py-2">
          ...
          <div className="flex min-w-0 items-baseline gap-2 font-instr text-[10px] uppercase tracking-kicker">
            ...
            {/* Relative-time chip — fresh writes show "刚刚", older ones
                resolve down to "N 分钟前" / "N 小时前" so the user knows
                whether a cell was just regenerated by an @-mention or
                left over from a previous run. */}
            {writeAgeMs !== null && (
              <span
                className={`normal-case tracking-normal ${
                  isFreshlyWritten ? byline.tintClass : 'text-stratum-muted'
                }`}
                title={`写入时间 ${new Date(writeAt).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })}`}
              >
                · {formatRelative(writeAgeMs)}
              </span>
            )}
            {nodeData?.metadata?.confidence && (
