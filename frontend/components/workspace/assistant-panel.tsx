  ...
  return (
    ...
    >
      <header className="border-b border-[#E3E6FF] px-6 py-4">
        ...
        <p className="mt-1 text-xs text-slate-500">输入问题，AI 帮您拆解为可执行的任务画布</p>
      </header>
        ...
        {messages.map((message) => (
          <div
            ...
            className={clsx(
              ...
              message.role === 'user'
                ? 'border-[#9B87F5]/20 bg-[#9B87F5]/5'
                : 'border-[#E3E6FF] bg-[#F7F8FF]'
            )}
          >
      ...
      <form className="border-t border-[#E3E6FF] px-6 py-4" onSubmit={handleSubmit}>
        <textarea
          ref={textareaRef}
          value={input}
          ...
          disabled={loading}
          className="h-24 w-full resize-none rounded-2xl border border-[#E3E6FF] bg-[#F7F8FF] px-4 py-3 text-sm text-slate-700 outline-none placeholder:text-slate-400 focus:border-[#9B87F5]/60 focus:ring-2 focus:ring-[#9B87F5]/20 disabled:opacity-50"
        />
        <div className="mt-3 flex items-center justify-between text-xs text-slate-500">
          <span>Shift + Enter 换行</span>
          <button
            className="rounded-full bg-gradient-to-r from-[#9B87F5] to-[#7A6EEF] px-4 py-1.5 text-xs font-medium text-white shadow-sm hover:from-[#8E7EEE] hover:to-[#6E60E6] disabled:opacity-50 disabled:cursor-not-allowed"
            type="submit"
