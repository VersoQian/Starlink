...
const STATUS_MAP: Record<TaskNodeData['status'], { label: string; color: string }> = {
  todo: { label: '待开始', color: '#A5ACF4' },
  'in-progress': { label: '进行中', color: '#8F9BFF' },
  ...
  return (
    <div data-testid="canvas-node-task">
      <NodeCard
        ...
        accent
        width={340}
        footer={<span className="text-xs text-[#7C80A9]">拖动到时间线以安排执行</span>}
      >
        <div className="flex items-center justify-between rounded-2xl bg-[#EEF0FF] px-4 py-3 text-xs text-slate-600">
